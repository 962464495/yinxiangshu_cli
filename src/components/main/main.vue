  ...
  <Layout style="height: 100%" class="main">
    <Sider hide-trigger collapsible :width="200" :collapsed-width="64" v-model="collapsed" class="left-sider"
      ...
      <side-menu accordion ref="sideMenu" :active-name="$route.name" :collapsed="collapsed" @on-select="turnToPage"
        ...
        <div class="logo-con">
          <h2 v-show="!collapsed">后台管理</h2>
          <h2 v-show="collapsed">后台</h2>
        </div>
      </side-menu>
    </Sider>
    <Layout>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      menuList () {
        var menu = this.$store.getters.menuList

        var backMenu = this.getMenu(menu)
      ...
      getMenu (menu) {
        ...
        for (var i = 0; i < menu.length; i++) {
          if (menu[i].children != undefined) {
            var subMenu = this.getMenu(menu[i].children)
            if (subMenu != []) {
              // TODO
              menu[i].children = subMenu
              backMenu.push(menu[i])
            } else {
              backMenu.push(menu[i])
            }
          } else {
